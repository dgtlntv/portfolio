...
excerpt:
    ...
    typography.
coverImage: "/images/projects/canonical-design-tokens/cover.png"
heroLocation: "cover"
asciiDarken: 0.89
stats:
...
When I joined Canonical Vanilla, Canonical's design system could probably be
described more as a CSS library. It lacked features that modern design systems
have, including a systematic approach to visual foundations. Visual foundations
existed most often as SCSS variables. However, they were also hard-coded
directly into the components.

At the same time, other teams had started implementing their own interpretations
of Vanilla in Flutter, and there was a Figma library with Vanilla components.
This situation made it quite manual and error-prone to ensure that these other
platforms visually matched our source of truth.

Canonical also recently changed its logo and its official typeface (from Ubuntu
to Ubuntu Sans), and the fact that the old logo and typeface can still be found
in various places across Canonical's presence shows that we lack the
infrastructure to efficiently implement these branding and visual changes across
Canonical's portfolio.

## Modernizing the design system

It was clear that what we needed was a centralized and systematic approach to
our visual foundations. In essence, we needed Design tokens and I pushed for and
led this effort to create Design tokens at Canonical.

![](/images/projects/canonical-design-tokens/inventory.png)
...
and so we quickly gained an understanding of the excisting needs.
...
team and stakeholders. We applied the same approach to design tokens. After
creating an initial specification draft, we invited the team to several
...
scaleable. So while working on design tokens we were not only looking to
translate our current visual foundations into a Design token framework but take
the opportunity and adopt a more systematic approach to visual foundations.

<FloatImage
    direction="left"
    content={
        <ColorPaletteMockup className="w-lg" />
    }
>

For colors for example, we previously didn't have a proper UI palette. Colors
were added on an as needed basis. We wanted to use the color token work as an
opportunity to move to a more systematic approach. We created a palette based on
Matthew Ström's article about how to create color palettes for design systems
where he desrcibes an approach where you do not create shades based on lightness
but based on contrast between shades (which makes creating accessible color
combinations easier).

</FloatImage>

At the time of writing, we had created primitive dimension tokens, primitive and
semantic typography tokens, and were almost finished with the color tokens. With
...
several implementation limitations that I was able to feed back into our token
...
`/build` folder, css output in the `/dist` folder and some of the tokens we had
created to this point in the `/tokens` folder.

<div className="relative -mx-4 my-8 sm:-mx-6 lg:-mx-8 xl:-mx-28">
    <GitHubCodeExplorer
        owner="dgtlntv"
        repo="design-tokens-reference"
        className="min-h-[500px]"
        defaultPath="tokens/canonical/sites/semantic/typography.tokens.json"
    />
</div>
