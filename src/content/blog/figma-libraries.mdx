...
excerpt: How Canonical's Design team rebuilt their Figma libraries, with practical guidelines on structure, performance, and maintenance processes.
coverImage: "https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/0953/Frame-419.png"
author: "Maximilian Blazek"
---

_This article was co-authored with Jan Ostrówka._
...
In revamping our Figma libraries, we wanted to structure them in a way that designers can easily understand and navigate the different libraries. Looking for inspiration, we came across the [Doctolib team's article on their Figma libraries](https://medium.com/doctolib/how-we-organise-our-design-system-libraries-to-help-doctolib-designers-use-more-than-70-000-c15237c81f6c) and Nathan Curtis' articles on [Figma libraries](https://medium.com/eightshapes-llc/figma-shared-libraries-d2dfbf9d1b8d) and [Design system tiers](https://medium.com/eightshapes-llc/design-system-tiers-2c827b67eae1).
...
![](https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/4a9f/Frame-416.png)
...
![](https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/6527/Frame-414.png)

If you want to learn more about the core and domain component paradigm, we recommend reading the [Doctolib article](https://medium.com/doctolib/how-we-organise-our-design-system-libraries-to-help-doctolib-designers-use-more-than-70-000-c15237c81f6c), it's well worth a read\! This approach, especially the domain components, allows us to achieve greater consistency between products.
...
![](https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/70d0/Frame-413.png)
...
While our new file structure improved navigation, we also wanted to tackle performance. [Figma's recent update](https://www.figma.com/blog/speeding-up-file-load-times-one-page-at-a-time/) has made file loading dynamic \- now pages load individually rather than all at once. However, it is still good practice to split large (library) files into several smaller files to ensure that we don't hit [Figma's 2 GB memory limit per file](https://help.figma.com/hc/en-us/articles/360040528173-Reduce-memory-usage-in-files#memory).
...
As described in [another Doctolib article](https://medium.com/doctolib/building-smarter-figma-components-crafting-for-efficiency-91704229643) (yes, we like their articles), there are a few usual suspects when it comes to the high number of layers.

#### Variants

When a component is placed in a design file, Figma loads every variant of a component into the file. This means that even if a component looks light in the layer view, it can be quite heavy if that component has many variants or variants with a high number of layers. You can test this by opening the [memory usage tool in Figma](https://help.figma.com/hc/en-us/articles/360040528173-Reduce-memory-usage-in-files#01H8HER66S8MW9E8E9HYKHXRVM).

![](https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/c9c1/Frame-411.png)
...
![](https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/9511/Frame-418.png)
...
We also **optimize all assets** (e.g. [svgo](https://github.com/svg/svgo), [oxipng](https://github.com/shssoichiro/oxipng)) before adding them to the library. This optimization targets two important performance factors. Firstly, it helps to comply with memory limitations. Secondly, it reduces download times when pages are opened on devices as each asset needs to be loaded for display.
...
![](https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/6473/Frame-412.png)
...
![](https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/3e02/Frame-415.png)
...
For new components, we ask contributors to create the proposed component in a separate file. There they can build the component, test it and get feedback from other designers. We provide [a checklist](https://www.figma.com/community/file/1435297834108003391/vanilla-core-component-library) of things they need to do in order for the component to be included in the library. Once they have completed the checklist, they contact a library maintainer to add the new component to the library. The library maintainer performs a final check and then adds it to the library.
...
![](https://res.cloudinary.com/canonical/image/fetch/c_limit,f_auto,q_auto,fl_sanitize,c_fill,w_10000,h_4000/https://ubuntu.com/wp-content/uploads/db22/Frame-417.png)

Before we started creating the new libraries, we also implemented a custom way to track the usage of Figma components and Figma libraries in our design team. We took inspiration from the article by the [Pinterest team](https://www.figma.com/blog/how-pinterests-design-systems-team-measures-adoption/) and the [Uber team's approach](https://youtu.be/PMz1DbaeoDE?feature=shared&t=213). With the code we wrote, we can track how often our designers use components in their files, which libraries those components come from, and how often they detach components.
...
We started this journey towards new libraries with the base core component library and the base assets library. Now that we have wrapped up the transition period for these libraries, we're sharing the files with the [Figma community](https://www.figma.com/community/file/1435297834108003391) and writing this blog post.
